...
The python environment is only required if you're willing to compute `gw_dist`, which is a distributional discrepancy measure (this is explained more in later sections). The environment is set up using the `reticulate` package.
...
This setup should only be done for the first use. For all subsequent usages, load the virtual environment that you've created using:
